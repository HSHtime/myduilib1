...
static struct mux_device* get_mux_device_for_id(int device_id)
{
  ...
  FOREACH(struct mux_device *cdev, &device_list ,mux_device *) {
...
static struct mux_connection* get_mux_connection(int device_id, struct mux_client *client)
{
	...
	//pthread_mutex_lock(&device_list_mutex);
	FOREACH(struct mux_device *dev, &device_list, mux_device *) {
		if(dev->id == device_id) {
			FOREACH(struct mux_connection *lconn, &dev->connections, mux_connection *) {
...
static int send_packet(struct mux_device *dev, enum mux_protocol proto, void *header, const void *data, int length)
{
	...
	usbmuxd_log(LL_NOTICE, "send_packet(%d, 0x%x, %p, %p, %d)", dev->id, proto, header, data, length);
...
static uint16_t find_sport(struct mux_device *dev)
{
	...
	while(1) {
		int ok = 1;
		FOREACH(struct mux_connection *conn, &dev->connections, mux_connection *) {
...
enum tcphdrFlag{ TH_SYN, TH_RST, TH_ACK };
...
int device_start_connect(int device_id, uint16_t dport, struct mux_client *client)
{
	...
	conn = (mux_connection *)malloc(sizeof(struct mux_connection));
...
void device_data_input(struct usb_device *usbdev, unsigned char *buffer, uint32_t length)
{
	struct mux_device *dev = NULL;
	//pthread_mutex_lock(&device_list_mutex);
	FOREACH(struct mux_device *tdev, &device_list, mux_device *) {
...
int device_add(struct usb_device *usbdev)
{
	...
	dev = (mux_device *)malloc(sizeof(struct mux_device));
	dev->id = id;
	dev->usbdev = usbdev;
...
int device_get_count(int include_hidden)
{
	...
	FOREACH(struct mux_device *dev, &dev_list, mux_device *) {
...
int device_get_list(int include_hidden, struct device_info **devices)
{
	...
	*devices = (device_info*)malloc(sizeof(struct device_info) * dev_list.capacity);
