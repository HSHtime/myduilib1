...
static int ppoll(struct pollfd *fds, int nfds, const  timespec *timeout)
{
	int to = timeout->tv_sec * 1000 + timeout->tv_nsec / 1000000;
...
static void parse_opts(int argc, char **argv)
{
	...
	while (1) {
		c = getopt_long(argc, argv, (char*)opts_spec, longopts, (int *) 0);
		if (c == -1) {
			break;
		}
...
int main(int argc, char *argv[])
{
	...
	usbmuxd_log(LL_NOTICE, "usbmuxd v starting up");
	should_exit = 0;
	should_discover = 0;
#ifndef WIN32
	set_signal_handlers();
...
#ifdef WIN32
	res = listenfd = socket_create(USBMUXD_SOCKET_PORT);
#else	
